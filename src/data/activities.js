/**
 * activities.js — 校园信息结构化数据
 *
 * 说明：
 * 1. 全部内容来自考核题目提供的 26 条模拟信息，未补充题目之外的任何事实。
 * 2. 时间统一使用本地时间字符串 "YYYY-MM-DDTHH:mm"。
 *    题目只给到「日期」而未给具体时刻的，一律填到 23:59 并在 deadlineText 中注明「具体时刻未注明」，
 *    避免向用户传递虚假的精确度。
 * 3. `missing` 只登记影响「能不能报名 / 去了会不会扑空」的关键字段，
 *    不把「发布方未署名」这类轻微问题也算进去，否则提示会失去意义。
 * 4. `supersedes` / `supersededBy` 表达通知之间的修订关系（09 修订 01、20 修订 03）。
 */
...
export const ACTIVITIES = [
  ...
  /* ------------------------------------------------------------------ 07 */
  {
    ...
    milestones: [
      ...
      { label: '作品提交截止', at: '2026-10-20T23:59' },
    ],
    notices: [
      {
        tone: 'warn',
  ...
  /* ------------------------------------------------------------------ 14 */
  {
    ...
    missing: ['活动地点', '预约截止时间', '报名入口'],
    riskLevel: 'warn',
    riskFlags: ['提交报名表不代表最终录取'],
    isNotice: false,
    addedAt: 14,
  ...
  /* ------------------------------------------------------------------ 24 */
  {
    ...
    missing: ['主办方', '活动时间', '活动地点', '具体内容', '报酬与结算方式'],
    riskLevel: 'danger',
    riskFlags: [
      '要求添加私人微信才能获取详情',
      '未提供任何可核实的主办方信息',
...
/** 补充通知：不单独作为卡片展示，而是合并进被修订的信息 */
export const NOTICE_IDS = ['09', '20'];

/** 会出现在主列表中的信息（排除补充通知） */
export const LISTED_ACTIVITIES = ACTIVITIES.filter((a) => !a.isNotice);
